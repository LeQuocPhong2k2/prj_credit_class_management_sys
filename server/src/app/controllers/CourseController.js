...
class CourseController {
  ...
  async getCourseNew(req, res) {
    ...
    try {
      ...
      res.status(200).json({
        message: 'Get course not clean successfully!!!',
        courseNotClean: major,
      })
    } catch (err) {
  ...
  async getCourseByStatus(req, res) {
    ...
    try {
      ...
      if (courseReCleans.length > 0) {
        res.status(200).json({
          message: 'Get course clean successfully!!!',
          courseReCleans: courseReCleans,
        })
      } else {
        res
          .status(200)
          .json({ message: 'ERR_404', courseReCleans: courseReCleans })
      }
    } catch (err) {
      res.status(500).json({ message: 'Server errorr' })
    }
  ...
  async getAllCourseOfMajor(req, res) {
    ...
    if (major) {
      ...
      res
        .status(200)
        .json({ message: 'Get course successfully!!!', courses: resultData })
    } else {
